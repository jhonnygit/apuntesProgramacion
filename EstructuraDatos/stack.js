//@see: http://blog.benoitvallon.com/data-structures-in-javascript/the-stack-data-structure/
//PILA
class Stack {
    constructor() {
      this.stack = [];
    }
  
    push(value) {
      this.stack.push(value);
    }
  
    pop() {
      return this.stack.pop();
    }
  
    peek() {
      return this.stack[this.stack.length - 1];
    }
  
    length() {
      return this.stack.length;
    }
  
    print() {
      console.log(this.stack.join(' '));
    }
  }
  
  const stack = new Stack();
  stack.push(1);
  stack.push(2);
  stack.push(3);
  stack.print(); 
  console.log('length is 3:', stack.length()); 
  console.log('peek is 3:', stack.peek()); 
  console.log('pop is 3:', stack.pop()); 
  stack.print(); 
  console.log('pop is 2:', stack.pop()); 
  console.log('length is 1:', stack.length()); 
  console.log('pop is 1:', stack.pop()); 
  stack.print(); 
  console.log('peek is undefined:', stack.peek()); 
  console.log('pop is undefined:', stack.pop()); 